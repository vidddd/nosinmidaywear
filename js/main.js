...
$('document').ready(function(){
	...
	if(pasos==0){
		if(currentTimeMs > 2280){
			mivideo.pause();

		}
	}else if(pasos==1){
		if(currentTimeMs > 4040){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==2){
		if(currentTimeMs > 5860){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==3){
		if(currentTimeMs > 7200){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==4){
		if(currentTimeMs > 8970){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==5){
		if(currentTimeMs > 10100){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==6){
		if(currentTimeMs > 12020){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==7){
		if(currentTimeMs > 13510){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==8){
		if(currentTimeMs > 15060){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==9){
		if(currentTimeMs > 16890){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}else if(pasos==10){
		if(currentTimeMs > 18080){
			mivideo.pause();
			mostrarBotonFinalColores();
		}
	}
...
function CambiarImagen(numero){
	...
	document.getElementById("grande").src = "images/gifs/"+numero+".gif";
  var num = parseInt(numero) - 1;
  document.getElementById("facebook2").href = data[num]['facebook'];
  document.getElementById("twitter2").href = data[num]['twitter'];
  document.getElementById("download2").href = data[num]['download'];
...
function aceptar_cookies(){
    ...
    document.cookie="cookies_daywear=aceptada; expires="+expire;

    var visit=GetCookie("cookies_daywear");
    if (visit==1){
      $('#cookies').toggle();
    }
}
